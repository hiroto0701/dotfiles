...
`~/.claude` は chezmoi で `private_dot_claude/` として管理されています。

### ファイルの編集ワークフロー

**通常ファイル**（agents/, rules/, reference/, skills/, commands/ など）は実ファイルを編集してから `re-add` します。

```bash
# 実ファイルを編集
vim ~/.claude/rules/core-essentials.md

# ソースに同期
chezmoi re-add ~/.claude/rules/core-essentials.md

# コミット
cd ~/.local/share/chezmoi && git add -p && git commit
```

**`mcp.json` はテンプレートファイルのため `re-add` 禁止**（テンプレート変数が上書きされる）。必ずソース側を編集してから `apply` します。

```bash
# テンプレートを編集
chezmoi edit ~/.claude/mcp.json

# 実ファイルに反映
chezmoi apply ~/.claude/mcp.json

# コミット
cd ~/.local/share/chezmoi && git add private_dot_claude/private_mcp.json.tmpl && git commit
```
...
1. `~/.config/chezmoi/chezmoi.toml` にキーを追加（ローカルのみ・gitに含まない）

```toml
[data]
    backlogApiKey = "..."
    newServiceApiKey = "新しいAPIキー"
```

2. テンプレートにキーを参照する形で追記

```bash
chezmoi edit ~/.claude/mcp.json
# "API_KEY": "{{ .newServiceApiKey }}" のように記述
```
...
`chezmoi init --apply` の前に `~/.config/chezmoi/chezmoi.toml` を作成しておく必要があります。

```toml
[data]
    backlogApiKey = "実際のAPIキー"
```
